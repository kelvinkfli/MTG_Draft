...
myVars.pickCounter = 1;
myVars.packCounter = 1;

myEvents.Tracker = function(){
    myVars.pickCounter += 1;
    $('#pick-count').html(`Pick ${myVars.pickCounter}`);
    if (myVars.pickCounter == 15) {
        myVars.pickCounter = 14;
    }
    myVars.packCounter += 1;
    $('#pack-count').html(`Pack ${myVars.packCounter}`)
    if (myVars.packCounter == 4) {
        myVars.packCounter = 0;
    }
}
...
myEvents.getPack = function(set){
    $.ajax({
        url: `http://noxoin.com:3000/generate/${set}/booster`,
        method: 'GET',
        dataType: 'json'
    }).then(function(cards){
...
myEvents.startDraft = function(){
    $('.start-draft').on('click', function(){
        ...
        setTimeout(myVars.delayStart, 1150);
    })
}
...
//make start new draft green button instead
//make arrow down buttons orange on hover